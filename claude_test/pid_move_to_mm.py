...
class PIDController:
    ...
    def compute(self, error_mm, dt_s):
        ...
        """
        p_term = self.kp * error_mm  # P(Proportional) Gain
        ...
        d_term = self.kd * self._filtered_derivative  # D(Derivative) Gain
        ...
        i_term = self.ki * self._integral  # I(Integral) Gain
...
def main():
    ...
    # Avoid target = 0 mm: hits the NOT (negative travel limit) switch
    # at the rail end, which mechanically absorbs overshoot and masks
    # real PID convergence behaviour. Use 30 mm as a safe interior
    # point — first iter from 50 mm overshoots ~10 mm, ending near
    # 20 mm, well clear of the 0 mm limit.
    targets_mm = [100.0, 50.0, 30.0]
    tolerance_mm = 0.05
    ...
    # P-only with kp=4.0 (lowered from 8.0). HIL run #3 showed
    # kp=8 + output_max=25 converges in 6-8 iter with target 30 mm
    # exhausting max_iterations. Linear-regime overshoot ratio
    # 1 + K·kp (K~0.06) predicts kp=4 → ratio 1.24× → ~3-4 total
    # iter to ±0.05 mm with safer margin. See ToDo.md Task 11.
    pid = PIDController(kp=4.0, ki=0.0, kd=0.0)
