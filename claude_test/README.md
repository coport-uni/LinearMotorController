...
| `pid_move_to_mm.py` | Iterative PID position controller (Issue #9 / Task 11): a discrete-time `PIDController` (anti-windup + EMA derivative + `[output_min, output_max]` saturation + deadband) whose output is the per-iteration speed into `move_relative_mm`. Per-tick stdout + timestamped `pid_log_*.csv`. Edit `targets_mm` / gains. | 2026-06-22: hardware-tuned to P-only kp=4.0 (ki=kd=0), output_max=25 r/min; converges to ±0.05 mm. Replaces the static [50,10,3,1,1] schedule. Offline logic in `test_pid_compute.py` (7/7). |
| `pid_measure_accuracy.py` | N-trial static-speed accuracy characterization (Issue #8): per trial PID-homes to a safe interior point, settles, then does one open-loop `move_relative_mm` at a fixed speed to the target and records the residual; prints mean/std/min/max. Logs `pid_accuracy_*.csv`. | 2026-06-22: measures raw `move_relative_mm` overshoot at a speed (plant behaviour), not PID convergence. Edit `target_mm`/`home_mm`/`num_trials`/`test_speed`. |
| `test_pid_compute.py` | Offline (no-hardware) sanity checks for `PIDController.compute()`: P-only output, saturation + sign, the `output_min` floor, deadband, and anti-windup (integral frozen while saturated). | 2026-06-22: 7/7 PASS. Run after editing the PID gains/logic in `pid_move_to_mm.py`. |
